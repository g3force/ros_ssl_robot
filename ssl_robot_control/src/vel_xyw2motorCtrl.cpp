...
void senderCallback(const ssl_robot_msg::vel_xyw::ConstPtr& vel)
{
  // fr, rr, rl, fl
  for(int i=0;i<4;i++)
  {
...
int main(int argc, char **argv)
{
  ...
  ctrl_pub[1] = n.advertise<std_msgs::Float64>("/ssl_robot/wheel_rr_vel_controller/command", 1);
  ctrl_pub[2] = n.advertise<std_msgs::Float64>("/ssl_robot/wheel_rl_vel_controller/command", 1);
  ctrl_pub[3] = n.advertise<std_msgs::Float64>("/ssl_robot/wheel_fl_vel_controller/command", 1);
